...
func hasHallRequestsChanged(oldHallRequests [config.NumFloors][2]bool, newHallRequests [config.NumFloors][2]bool) bool {
...
func hasCabRequestsChanged(oldCabRequests [config.NumFloors]bool, newCabRequests [config.NumFloors]bool) bool {
	for i := 0; i < config.NumFloors; i++ {
...
func AssignRequests(ch_myWorldView <-chan worldview.MyWorldView, ch_hallRequest chan<- [config.NumFloors][2]bool, ch_cabRequests chan<- [config.NumFloors]bool, localID string) {
	...
	for {
		...
		case myWorldView := <-ch_myWorldView:
			...
			if hasHallRequestsChanged(oldHallRequests, hallRequests) {
				ch_hallRequest <- hallRequests
				oldHallRequests = hallRequests
			}
			if hasCabRequestsChanged(oldCabRequests, cabRequests) {
